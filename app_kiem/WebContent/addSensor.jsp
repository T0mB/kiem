...
$(document).ready(function(){	
	$("#sel1").change(function() {
		...
		$.getJSON('http://localhost:8080/app_kiem/api/sensorservice/getsensorlist/' + $("#sel1 option:selected").text(), function(sensor) {
	        $.each(sensor, function(index) {
	            console.log(sensor[index].id);
	            $('#sel2').append($("<option />").val(sensor[index].idsensor).text(sensor[index].idsensor)); //.text(sensor[index].type)
	        });
	    });
	});
});
...
$(document).ready(function(){	
	$("#sel2").change(function() {
		$.getJSON('http://localhost:8080/app_kiem/api/sensorservice/getsensorlist/' + $("#sel1 option:selected").text(), function(sensor) {
	        $.each(sensor, function(index) {
	            if(sensor[index].idsensor == $("#sel2").val()){
					console.log(sensor[index].type);
					$('#typesensor').val(sensor[index].type);	
	            }
	        });
	    });
	});
...
$(document).ready(function(){	
	$("#sel3").change(function() {
		console.log($("#sel1 option:selected").text());
		$.getJSON('http://localhost:8080/app_kiem/api/sensorservice/getsensorlist/' + $("#sel3 option:selected").text(), function(sensor) {
	        $.each(sensor, function(index) {
	            console.log(sensor[index].id);
	            $('#sel4').append($("<option />").val(sensor[index].idsensor).text(sensor[index].idsensor)); 
	        });
	    });
	});
});		
...
<script>
	function create() {
		...
		var t = $('#inputtype').val();
		var d = $('#db').val();
		var url = 'http://localhost:8080/app_kiem/api/sensorservice/createsensor/'
				+ d + '+' + t;
	...
	function update() {
		...
		var uri = 'http://localhost:8080/app_kiem/api/sensorservice/updateSensor/'
				+ db + '+' + id + '+' + tp;
		alert(tp);
	...
	function delete1() {
		...
		var uri = 'http://localhost:8080/app_kiem/api/sensorservice/deletesensor/'
				+ db + '+' + id;
...
<body>
	...
	<div class="divd">
		...
		<form id="formupdate">
			...
			<label for="sel1">Select id:</label> <select class="form-control"
				id="sel2">
				<option>choose a sensor id</option>
			</select> 
			<br/>
			<input name="type" id="typesensor" type="text"
				placeholder="type of sensor" size="45">
